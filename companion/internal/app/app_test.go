...
func TestRuntimeWiresSharedStoresWithoutChangingHostIdentityOrConfirmedResult(t *testing.T) {
	...
	dependencies := Dependencies{PairingStore: pairingStore, PromptStore: promptStore, EventStore: eventStore, Random: rand.Reader}
...
func TestRuntimeRejectsMissingStoresAndRandomSource(t *testing.T) {
	config := Config{Version: 1, ComputerName: "Computer", ListenHost: "100.64.0.10", ListenPort: 9443, Projects: []projects.Config{{ID: "main", DisplayName: "Main", Path: canonicalTempDir(t)}}}
	valid := Dependencies{PairingStore: pairing.NewMemoryStore(), PromptStore: promptqueue.NewMemoryStore(), EventStore: eventjournal.NewMemoryStore(eventjournal.Limits{MaxEvents: 4, MaxBytes: 1024}), Random: rand.Reader}
