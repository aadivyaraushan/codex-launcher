...
// lost reply actually looks like: the request arrived, Todoist may well have
...
// A lost reply to a write (task creation) is ambiguous: Todoist may have
// already created the task. A lost reply to a read never is — nothing
...
func TestHTTPClientClassifiesLostRepliesByReadOrWrite(t *testing.T) {
	t.Run("a write whose reply never arrives is an unknown outcome", func(t *testing.T) {
		...
		_, err := client.CreateTask(context.Background(), CreateTask{Content: "Buy oat milk"})
	...
	t.Run("a write Todoist answered with a non-2xx status is a plain failure", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
