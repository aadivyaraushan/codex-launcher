...
// lost reply actually looks like: the request arrived, Calendar may well
// have acted on it, and only the answer went missing.
//
...
// A lost reply to a write (event creation) is ambiguous: Calendar may have
// already created the event. A lost reply to a read never is — nothing
...
func TestHTTPClientClassifiesLostRepliesByReadOrWrite(t *testing.T) {
	t.Run("a write whose reply never arrives is an unknown outcome", func(t *testing.T) {
		...
		_, err := client.CreateEvent(context.Background(), CreateEvent{Summary: "Standup", Start: "2026-01-01T09:00:00Z", End: "2026-01-01T09:30:00Z"})
	...
	t.Run("a read whose reply never arrives is a plain failure", func(t *testing.T) {
		...
		client := NewHTTPClient(server.URL, staticToken("access-secret"), server.Client(), slog.New(slog.NewTextHandler(io.Discard, nil)))
		_, err := client.ListEvents(context.Background(), "standup")
	...
	t.Run("a write Calendar answered with a non-2xx status is a plain failure", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
