...
var argOrder = map[string][]string{
	ScriptListExists:     {"list"},
	ScriptCreateList:     {"list"},
	ScriptCreateReminder: {"list", "title", "body"},
	ScriptReadReminders:  {"list"},
}
...
// ScriptCreateReminder — the one script here that mutates something a
// duplicate run would double, unlike ScriptCreateList, whose caller always
// checks existence first and so is safe to simply retry — and only when ctx
// was canceled or timed out, meaning Operator killed osascript without
// learning whether the Apple Event it had already sent reached
// Reminders.app. An ordinary non-zero exit (osascript ran and told us it
// failed) is never ambiguous.
func classifyOsascriptFailure(ctx context.Context, scriptName string, err error) error {
	if scriptName == ScriptCreateReminder && ctx.Err() != nil {
