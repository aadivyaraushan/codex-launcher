...
// lost reply actually looks like: the request arrived, Teams may well have
...
// A lost reply to a write (a chat message) is ambiguous: Teams may have
// already posted it. A lost reply to a read never is — nothing changed, so
// it is always safe to just retry.
func TestHTTPClientClassifiesLostRepliesByReadOrWrite(t *testing.T) {
	t.Run("a write whose reply never arrives is an unknown outcome", func(t *testing.T) {
		...
		_, err := client.SendMessage(context.Background(), SendMessage{ChatID: "19:chat", Body: "hello"})
	...
	t.Run("a write Teams answered with a non-2xx status is a plain failure", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
