...
func (c *Client) Model(ctx context.Context, utterance string) ([]byte, error) {
	request := responseRequest{
		...
		Instructions: strings.Join([]string{
			...
			"For Spotify prepare-and-open, use app_class media and app_named spotify with verb play (listen/search) or write (playlist or library edit intent). Open only — never claim playback control or playlist changes.",
			...
			"For YouTube prepare-and-open, use app_class media and app_named youtube with verb play (open/search title) or read (browse/search). Open only — never claim played.",
			...
			"For Messages prepare-and-open (plan iMessage row on Android → Google Messages compose hand-off), use app_class messaging and app_named messages with verb compose. Open only — never claim the message was sent.",
			"For Discord prepare-and-open (servers and DMs), use app_class messaging and app_named discord with verb compose. Open only — never claim the message was sent.",
