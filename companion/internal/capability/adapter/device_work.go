...
// DeviceWorkError marks a capability action that Confirm cannot finish by
// itself, because the acting half of it has to happen on the phone.
//
// This is not an OutcomeUnknownError. Every other capability decides and
// acts inside the one call to Confirm, and the result is already known by
// the time it returns. A notification reply cannot work that way: routing
// the reply needs the language model, which runs on the Mac, but the reply
// box it has to be typed into lives inside an Android notification, which
// only the phone can reach. So Confirm returns this error to say "the
// decision is made, but somebody else still has to carry it out" — not to
// say that anything has gone wrong or that any outcome is in doubt.
//
