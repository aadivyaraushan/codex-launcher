...
class ProtocolContractTest {
    // The predetermined-function pipeline is gone (Phase 8), and its five
    // wire names left with it — disconnecting an app is now the agent's
    // gated disconnect verb, not a frame. A frame carrying any of the five
    // names, including one that was valid for years, must fail exactly like
    // a name that never existed: a name the codec accepts but nobody handles
    // is a frame that silently disappears.
    @Test
    fun `capability frames left the protocol with their pipeline`() {
        val rejected =
            listOf(
        ...
        rejected.forEach { frame -> assertTrue(frame, runCatching { ProtocolCodec.decodeText(frame) }.isFailure) }
    }
